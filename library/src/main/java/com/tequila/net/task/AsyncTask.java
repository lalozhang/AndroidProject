 ...
 * 
 * so that threading behavior on all OS versions is the same and we can tweak behavior by using executeOnExecutor() if
 * needed.
 * 
 * There are 3 changes in this copy of AsyncTask: -pre-HC a single thread executor is used for serial operation
 * (Executors.newSingleThreadExecutor) and is the default -the default THREAD_POOL_EXECUTOR was changed to use
 * DiscardOldestPolicy -a new fixed thread pool called DUAL_THREAD_EXECUTOR was added
 * *************************************
 * 
 * <p>
 ...
 * 
 * <p>
 * AsyncTask is designed to be a helper class around {@link Thread} and {@link Handler} and does not constitute a
 * generic threading framework. AsyncTasks should ideally be used for short operations (a few seconds at the most.) If
 * you need to keep threads running for long periods of time, it is highly recommended you use the various APIs provided
 * by the <code>java.util.concurrent</code> pacakge such as {@link Executor}, {@link ThreadPoolExecutor} and
 * {@link FutureTask}.
 * </p>
 * 
 * <p>
 ...
 * {@link android.os.Build.VERSION_CODES#JELLY_BEAN}.</li>
 ...
 * {@link android.os.Build.VERSION_CODES#DONUT}, this was changed to a pool of threads allowing multiple tasks to
 * operate in parallel. Starting with {@link android.os.Build.VERSION_CODES#HONEYCOMB}, tasks are executed on a single
 * thread to avoid common application errors caused by parallel execution.
 * </p>
 * <p>
 * If you truly want parallel execution, you can invoke
 * {@link #executeOnExecutor(Executor, Object[])} with {@link #THREAD_POOL_EXECUTOR}.
 * </p>
 */
public abstract class AsyncTask<Params, Progress, Result> {
     ...
     * particular process.
     */
    public static final Executor SERIAL_EXECUTOR = CompatUtil.hasHoneycomb() ? new SerialExecutor() : Executors
...
//                    QLog.w(LOG_TAG, e);
    ...
    /**
     * Returns <tt>true</tt> if this task was cancelled before it completed normally. If you are calling
     * {@link #cancel(boolean)} on the task, the value returned by this method should be checked periodically from
     ...
     * 
     * @throws IllegalStateException If {@link #getStatus()} returns either {@link Status#RUNNING} or
     ...
     * This method must be invoked on the UI thread.
     * 
     * @param exec The executor to use. {@link #THREAD_POOL_EXECUTOR} is available as a convenient process-wide thread
    ...
    public final AsyncTask<Params, Progress, Result> executeOnExecutor(Executor exec, Params... params) {
        if (mStatus != Status.PENDING && mStatus != null) {
            switch (mStatus) {
            case RUNNING:

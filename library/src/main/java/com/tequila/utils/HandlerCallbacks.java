...
public class HandlerCallbacks {
	...
	public final static int MESSAGE_SERVER_ERROR = 1007;
        ...
        public boolean handleMessage(Message msg) {
            if (msg.obj instanceof NetworkParam) {
                ...
                case TaskStatus.ERROR:
					...
					break;
				case TaskStatus.SERVER_ERROR:
                    synchronized (this) {
                        if (listener != null) {
                            listener.onNetError(param, MESSAGE_SERVER_ERROR);
                        }
                    }
                    break;
                case TaskStatus.SUCCESS:
                    IServiceMap type = param.key;
                    if (type == null) {
                        synchronized (this) {
                            if (listener != null) {
                ...
                case TaskStatus.END:
                    ...
                    break;
				case TaskStatus.CACHE_HITTED:
					synchronized (this){
						if(listener != null){
							listener.onCacheHit(param);
						}
					}
					break;
                }
